...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['sell_products'])) {
    ...
    $discounts = $_POST['discounts']; // Discounts per product
    $total_amount = 0;
    ...
    try {
        ...
        for ($i = 0; $i < count($products); $i++) {
            ...
            $stmt = $conn->prepare("SELECT dry_stock, polish_cut_stock, polish_jagged_stock, damage_stock, 
                        ...
                        <div class="row mb-3 product-row">
                            <div class="col-md-3">
                                ...
                                <select name="products[]" class="form-control product-select" required onchange="updateSubtotal()">
                                    <?php
                                    $stmt = $conn->prepare("SELECT * FROM products WHERE dry_stock > 0 OR polish_cut_stock > 0 OR polish_jagged_stock > 0 OR damage_stock > 0");
                                    $stmt->execute();
                                    ...
                                    while ($row = $products->fetch_assoc()) {
                                        echo "<option value='{$row['id']}' 
                                            ...
                                            {$row['name']} (Dry: {$row['dry_stock']} @ {$row['dry_price']}, Polish-Cut: {$row['polish_cut_stock']} @ {$row['polish_cut_price']}, Polish-Jagged: {$row['polish_jagged_stock']} @ {$row['polish_jagged_price']}, Damage: {$row['damage_stock']} @ {$row['damage_price']})
                                        </option>";
                                    }
                                    $stmt->close();
                                    ?>
